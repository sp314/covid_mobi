...
load("~/Desktop/school/math453/covid_mobi/data/rides_19_q1.RData")
load("~/Desktop/school/math453/covid_mobi/data/rides_19_q2.RData")
...
rides_20_h1clean %>%
	...
	summarise(
		n = group_size(.),
		total_rides = nrow(rides_20_h1clean),
		ride_proportion = n/total_rides) %>%
	ggplot() +
	geom_line(mapping = aes(x = day, y = ride_proportion, color=weekend))
```
...
rides_20_h1clean %>%
	...
	geom_sf(mapping = aes(fill = ride_proportion)) + geom_sf_text(aes(label = PULocationID), colour = "white") +
	theme_map +
